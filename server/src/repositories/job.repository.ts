...
export class JobRepository {
  ...
  async setConcurrency(queueName: QueueName, concurrency: number): Promise<void> {
    ...
    // if (!runner) {
    //   this.logger.error(`Failed to set concurrency for queue: ${queueName}`);
    //   return;
    // }
    // await runner.stop();
  ...
  getJobOptions(item: JobItem): TaskSpec | undefined {
    switch (item.name) {
      ...
      case JobName.STORAGE_TEMPLATE_MIGRATION_SINGLE: {
        // Setting a queue name forces it to run serially.
        return { queueName: QueueName.STORAGE_TEMPLATE_MIGRATION };
      }
